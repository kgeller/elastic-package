...
func WriteDocumentationFiles(pkgRootDir string, llmResponseSentences ResponseSentences) error {
	// Define path and create directories for documentation files
	outputFileDir := filepath.Join(pkgRootDir, "_dev", "build", "docs", "sections")
...
func GenerateContentWithBedrock(packageName string) (ResponseSentences, error) {
	...
	* Basic example 
	...
	Please return the response in one line JSON format so that overview and setup instructions are separate keys. Do not add any additional text or comments.
